...
import { FileText, Plus, Trash2, Download, Sparkles, Loader2 } from "lucide-react";
...
export default function Invoice() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      doc.text("Total", 165, yPos, { align: "right" });
      ...
      validItems.forEach((item) => {
        ...
        doc.text(formatCurrency(item.quantity * item.unitPrice), 165, yPos, { align: "right" });
      ...
      doc.rect(120, yPos - 5, pageWidth - margin - 120, 12, 'F');
      
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("TOTAL DUE:", 125, yPos + 2);
      doc.text(formatCurrency(calculateTotal()), 165, yPos + 2, { align: "right" });
      ...
      // Save the PDF
      doc.save(`${invoiceNumber.current}-${clientName.replace(/\s+/g, '-')}.pdf`);
      
      toast({
        title: "Invoice Downloaded!",
        description: `${invoiceNumber.current} has been saved to your device.`,
      });
    } catch (error) {
              ...
              <div className="bg-background rounded-lg border border-border p-6">
                ...
                <div className="mt-6 pt-5 border-t border-border">
                  <p className="text-xs text-muted-foreground text-center">
                    Generated by MtaaBiz AI • Payment due within 30 days
                  </p>
                </div>
